...
'Converts the birth year and death year from being treated as string/characters into a number' 
NAME_BASICS <- NAME_BASICS |> 
  mutate(birthYear = as.numeric(birthYear), 
         deathYear = as.numeric(deathYear)) 
'Task 1 Cleaning the columns and ensuring they are the correct type' 
TITLE_BASICS <- TITLE_BASICS |> 
...
'Adding in a glimpse to check if treated as updated types' 
'Task 2' 
'1.' 
print(TITLE_BASICS |> 
...
print (TITLE_RATINGS |> 
         ...
         rename(Rating = averageRating,Number_of_votes = numVotes,Episode=EpisodeTitle,Series=SeriesTitle))
```
...
print(TITLE_EPISODES |> 
        ...
        select(-parentTconst) |>
        rename(Number_of_episodes=Countrows,Tv_series=primaryTitle,Number_of_votes=numVotes)  |>
        head(10))
```
...
print(ggplot(GRAPH_INFORMATION,aes(x = seasonNumber, y = averageRating)) + 
...
print(Successful_Movies |> 
        select(-tconst,-titleType,-originalTitle,-isAdult,-endYear,-runtimeMinutes) |>
        head(5))
...
print(TITLE_RATINGS |> 
        ...
        rename(Release_Year = startYear, Title = primaryTitle) |>
        head(3))
```
...
```{r}
print(ggplotly(ggplot(genre_counts, aes(x = decade, y = count, fill = genres)) +
           ...
           ggtitle(label = "Number of Movies by Genre and Decade", subtitle =                "Hover over a bar to see the genre")))
...
```{r}
ggplotly(ggplot(genre_counts_total, aes(x = decade, y = percentage_of_count, color = genres,group = genres, text = paste0(round(percentage_of_count*100, 0), "%"))) +
           geom_line() + 
           geom_point() + 
           labs(title = "Percentage of Total Successes by Genre and Year", x = "Year", y = "Percentage of Total Successes") + 
           scale_x_continuous(breaks = seq(1920, 2020, by = 10), labels = paste0(seq(1920, 2020, by = 10), "s")) + 
           scale_y_continuous(labels = scales::percent) + ggtitle(label = "Number of Movies by Genre and Decade", subtitle = "Hover over a bar to see the genre"))
...
ggplotly(ggplot(Actor_success |>
          ...
          ggtitle(label = "Number of Advenutre movies votes by Year",
                  subtitle = "Hover over a bar to see the genre"))
```
...
```{r}
print(ggplotly(ggplot(TITLE_RATINGS|>
                  inner_join(TITLE_BASICS |>
                               separate_longer_delim(genres, ",")|>
                               filter(genres == "Adventure") |>
                               filter(titleType == "movie") |>
